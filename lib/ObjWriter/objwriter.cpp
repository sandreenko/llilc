...
void ObjectWriter::EmitFunctionSymbolDef(const char *SymbolName) {
  auto &OST = *Asm->OutStreamer;
  MCContext &OutContext = OST.getContext();

  MCSymbol *Sym = OutContext.getOrCreateSymbol(Twine(SymbolName));
  MCSymbolAttr Attribute;
  if (GetTriple().getObjectFormat() == Triple::ELF) {
    Attribute = MCSA_ELF_TypeFunction;
  } else {
    Attribute = MCSA_Global;
  }
  OST.EmitSymbolAttribute(Sym, Attribute);
  OST.EmitLabel(Sym);
}
