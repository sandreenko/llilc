...
Triple ObjectWriter::GetTriple()
{
	...
	{
		TheTriple = Triple(TheTriple.getArchName(), TheTriple.getVendorName(), "darwin",
			TheTriple.getEnvironmentName()); // it is workaround for llvm bug
...
bool ObjectWriter::init(llvm::StringRef ObjectFilePath)
{
	...
	const Target* TheTarget = TargetRegistry::lookupTarget(TripleName, TargetError);
	if (!TheTarget)
	{
		return error("Unable to create target for " + ObjectFilePath + ": " + TargetError);
	}

	std::error_code EC;
	OS.reset(new raw_fd_ostream(ObjectFilePath, EC, sys::fs::F_None));
	if (EC)
		return error("Unable to create file for " + ObjectFilePath + ": " + EC.message());
	...
	MS = TheTarget->createMCObjectStreamer(TheTriple, *MC, *MAB, *OS, MCE, *MSTI, RelaxAll,
		/*IncrementalLinkerCompatible*/ true,
		/*DWARFMustBeAtTheEnd*/ false);
	if (!MS)
		return error("no object streamer for target " + TripleName);

	TM.reset(TheTarget->createTargetMachine(TripleName, MCPU, FeaturesStr, TargetOptions(), None));
	if (!TM)
...
void ObjectWriter::SwitchSection(const char* SectionName, CustomSectionAttributes attributes, const char* ComdatName)
{
	...
	else
	{
		SectionKind Kind = (attributes & CustomSectionAttributes_Executable)
			? SectionKind::getText()
			: (attributes & CustomSectionAttributes_Writeable) ? SectionKind::getData()
		...
		case Triple::MachO:
		{
			...
			Section =
				OutContext.getMachOSection((attributes & CustomSectionAttributes_Executable) ? "__TEXT" : "__DATA",
		...
		case Triple::COFF:
		{
			...
			{
				Characteristics |= COFF::IMAGE_SCN_CNT_INITIALIZED_DATA | COFF::IMAGE_SCN_MEM_WRITE;
			}
			else
			{
			...
			{
				Section = OutContext.getCOFFSection(SectionName, Characteristics | COFF::IMAGE_SCN_LNK_COMDAT, Kind,
...
const MCSymbolRefExpr* ObjectWriter::GetSymbolRefExpr(const char* SymbolName, MCSymbolRefExpr::VariantKind Kind)
{
...
int ObjectWriter::EmitSymbolRef(const char* SymbolName, RelocType RelocationType, int Delta)
{
	auto&      OST = static_cast<MCObjectStreamer&>(*Asm->OutStreamer);
	MCContext& OutContext = OST.getContext();
	...
	if (IsPCRelative)
	{
		...
		TargetExpr = MCBinaryExpr::createSub(TargetExpr, MCConstantExpr::create(Size, OutContext), OutContext);
	}

	if (Delta != 0)
	{
		TargetExpr = MCBinaryExpr::createAdd(TargetExpr, MCConstantExpr::create(Delta, OutContext), OutContext);
	}
	OST.EmitValueImpl(TargetExpr, Size, SMLoc(), IsPCRelative);
	return Size;
}

void ObjectWriter::EmitWinFrameInfo(const char* FunctionName,
	int         StartOffset,
	int         EndOffset,
	const char* BlobSymbolName)
{
	...
	const MCExpr* BaseRefRel = GetSymbolRefExpr(FunctionName, MCSymbolRefExpr::VK_COFF_IMGREL32);
	...
	OST.EmitValue(GetSymbolRefExpr(BlobSymbolName, MCSymbolRefExpr::VK_COFF_IMGREL32), 4);
}
...
void ObjectWriter::EmitCFILsda(const char* LsdaBlobSymbolName)
{
	...
	MCAssembler& MCAsm = OST.getAssembler();
	MCAsm.registerSymbol(*T);
	OST.EmitCFILsda(T, llvm::dwarf::Constants::DW_EH_PE_pcrel | llvm::dwarf::Constants::DW_EH_PE_sdata4);
}
...
void ObjectWriter::EmitCFICode(int Offset, const char* Blob)
{
	...
	case CFI_ADJUST_CFA_OFFSET:
		assert(CfiCode->DwarfReg == DWARF_REG_ILLEGAL && "Unexpected Register Value for OpAdjustCfaOffset");
	...
	case CFI_DEF_CFA_REGISTER:
		assert(CfiCode->Offset == 0 && "Unexpected Offset Value for OpDefCfaRegister");
...
void ObjectWriter::EmitLabelDiff(MCStreamer& Streamer, const MCSymbol* From, const MCSymbol* To, unsigned int Size)
{
	MCSymbolRefExpr::VariantKind Variant = MCSymbolRefExpr::VK_None;
	MCContext&                   Context = Streamer.getContext();
	const MCExpr *               FromRef = MCSymbolRefExpr::create(From, Variant, Context),
		*ToRef = MCSymbolRefExpr::create(To, Variant, Context);
	const MCExpr* AddrDelta = MCBinaryExpr::create(MCBinaryExpr::Sub, ToRef, FromRef, Context);
	Streamer.EmitValue(AddrDelta, Size);
}

void ObjectWriter::EmitSymRecord(MCObjectStreamer& OST, int Size, SymbolRecordKind SymbolKind)
{
	RecordPrefix Rec;
...
void ObjectWriter::EmitCOFFSecRel32Value(MCObjectStreamer& OST, MCExpr const* Value)
{
...
void ObjectWriter::EmitVarDefRange(MCObjectStreamer& OST, const MCSymbol* Fn, LocalVariableAddrRange& Range)
{
	const MCSymbolRefExpr* BaseSym = MCSymbolRefExpr::create(Fn, OST.getContext());
	const MCExpr*          Offset = MCConstantExpr::create(Range.OffsetStart, OST.getContext());
	const MCExpr*          Expr = MCBinaryExpr::createAdd(BaseSym, Offset, OST.getContext());
	EmitCOFFSecRel32Value(OST, Expr);
	OST.EmitCOFFSectionIndex(Fn);
	OST.EmitIntValue(Range.Range, 2);
}

void ObjectWriter::EmitCVDebugVarInfo(MCObjectStreamer& OST,
	const MCSymbol*   Fn,
	...
	for (int I = 0; I < NumVarInfos; I++)
	{
		...
		for (const auto& Range : Var.Ranges)
		{
			...
			case ICorDebugInfo::VLT_REG_FP:
			{
				...
				if (Range.loc.vlReg.vlrReg >= sizeof(cvRegMapAmd64) / sizeof(cvRegMapAmd64[0]))
				{
					break;
				}
				SymbolRecordKind SymbolKind = SymbolRecordKind::DefRangeRegisterSym;
				unsigned         SizeofDefRangeRegisterSym =
					sizeof(DefRangeRegisterSym::Hdr) + sizeof(DefRangeRegisterSym::Range);
			...
			case ICorDebugInfo::VLT_STK:
			{
				...
				if (Range.loc.vlStk.vlsBaseReg >= sizeof(cvRegMapAmd64) / sizeof(cvRegMapAmd64[0]))
				{
					break;
				}

				assert(Range.loc.vlStk.vlsBaseReg < sizeof(cvRegMapAmd64) / sizeof(cvRegMapAmd64[0]) &&
				...
				unsigned         SizeofDefRangeRegisterRelSym =
					sizeof(DefRangeRegisterRelSym::Hdr) + sizeof(DefRangeRegisterRelSym::Range);
				...
				OST.EmitBytes(StringRef((char*)&DefRangeRegisterRelSymbol.Hdr, Length));
...
void ObjectWriter::EmitCVDebugFunctionInfo(const char* FunctionName, int FunctionSize)
{
	auto&                   OST = static_cast<MCObjectStreamer&>(*Asm->OutStreamer);
	MCContext&              OutContext = OST.getContext();
	const MCObjectFileInfo* MOFI = OutContext.getObjectFileInfo();
	...
	MCSymbol *SymbolsBegin = OutContext.createTempSymbol(), *SymbolsEnd = OutContext.createTempSymbol();
	...
	OST.EmitLabel(SymbolsBegin);
	{
		...
		unsigned HeaderSize = sizeof(ProcSymbol.Parent) + sizeof(ProcSymbol.End) + sizeof(ProcSymbol.Next) +
			sizeof(ProcSymbol.CodeSize) + sizeof(ProcSymbol.DbgStart) + sizeof(ProcSymbol.DbgEnd) +
...
void ObjectWriter::EmitDebugFunctionInfo(const char* FunctionName, int FunctionSize)
{
	auto&                   OST = static_cast<MCObjectStreamer&>(*Asm->OutStreamer);
	MCContext&              OutContext = OST.getContext();
	const MCObjectFileInfo* MOFI = OutContext.getObjectFileInfo();
...
void ObjectWriter::EmitDebugVar(
	char* Name, int TypeIndex, bool IsParm, int RangeCount, ICorDebugInfo::NativeVarInfo* Ranges)
{
	assert(RangeCount != 0);
...
void ObjectWriter::EmitDebugLoc(int NativeOffset, int FileId, int LineNumber, int ColNumber)
{
	auto&                   OST = static_cast<MCObjectStreamer&>(*Asm->OutStreamer);
	MCContext&              OutContext = OST.getContext();
	const MCObjectFileInfo* MOFI = OutContext.getObjectFileInfo();
	...
	{
		OST.EmitCVFuncIdDirective(FuncId);
		OST.EmitCVLocDirective(FuncId, FileId, LineNumber, ColNumber, false, true, "", SMLoc());
	}
	else
	{
		OST.EmitDwarfLocDirective(FileId, LineNumber, ColNumber, 1, 0, 0, "");
		MCSymbol *LineSym = OST.getContext().createTempSymbol();
		// Set the value of the symbol to use for the MCDwarfLineEntry.
		OST.EmitLabel(LineSym);

		// Get the current .loc info saved in the context.
		const MCDwarfLoc &DwarfLoc = OST.getContext().getCurrentDwarfLoc();

		// Create a (local) line entry with the symbol and the current .loc info.
		MCDwarfLineEntry LineEntry(LineSym, DwarfLoc);

		// clear DwarfLocSeen saying the current .loc info is now used.
		OST.getContext().clearDwarfLocSeen();

		// Add the line entry to this section's entries.
		OST.getContext()
			.getMCDwarfLineTable(OST.getContext().getDwarfCompileUnitID())
			.getMCLineSections()
			.addLineEntry(LineEntry, OST.getCurrentSectionOnly());
	}
}
